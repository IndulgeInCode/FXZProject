...
with graph.as_default():
    ...
    y_tru=tf.placeholder(dtype=tf.float32 ,shape=[BATCH_SIZE,class_num],name="pred_placeholder")
    ...
    lstm_forward = rnn.MultiRNNCell(cells=[lstm_forward_1, lstm_forward_2, lstm_forward_3, lstm_forward_4])
    ...
    lstm_backward = rnn.MultiRNNCell(cells=[lstm_backward_1, lstm_backward_2, lstm_backward_3,  lstm_backward_4])
...
#深度LSTM网络
    ...
    cross_loss = tf.losses.softmax_cross_entropy(onehot_labels=y_tru,
                                                 logits=h)
    # print(loss.shape)
    ...
    for epoch in range(1,EPOCH+1):
        ...
        for j in range(TRAIN_EXAMPLES/BATCH_SIZE):
            _, accu, train_loss=sess.run(
                    fetches=(optimizer, accuracy, cross_loss),
                    feed_dict={
